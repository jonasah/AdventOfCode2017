...
namespace AdventOfCode2017
{
    public static class Day4
    {
        ...
        {
            Util.WriteResult(4, 1, CalculatePart1(PuzzleInput));
        }

        public static void SolvePart2()
        {
            Util.WriteResult(4, 2, CalculatePart2(PuzzleInput));
        }

        public static int CalculatePart1(string input)
        {
            var passphrases = ConvertInput(input);

            // check if unique word count equals total word count
            return passphrases
        ...
        public static int CalculatePart2(string input)
        {
            ...
            return passphrases
                .Count(words =>
                {
                    // 'sort' each word and then check if unique word count equals total word count
                    var sortedWords = words.Select(word => word.OrderBy(c => c).Join(string.Empty));
                    return sortedWords.Distinct().Count() == words.Count;
                });
        }

        private static List<List<string>> ConvertInput(string input)
        {
            // first split by row, then split by word
            return input
        ...
        private static readonly string PuzzleInput = @"kvvfl kvvfl olud wjqsqa olud frc
